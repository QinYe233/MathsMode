...
describe('vectorGraphType', () => {
  ...
  it('箭头头部为填充同色的三角形 polygon', () => {
    ...
    expect(head.getAttribute('stroke')).toBe('#f59e0b');
  });
  ...
  it('重复重绘不复制元素且坐标更新', () => {
    ...
    document.body.appendChild(svg);
    const plotter = vectorGraphTypeBuilder(chart);
    plotter(select(g).datum({ vector: [3, 4], color: '#2563eb', index: 0 }));
    plotter(select(g).datum({ vector: [5, 1], color: '#2563eb', index: 0 }));
    expect(g.querySelectorAll('line.vector-arrow').length).toBe(1);
    expect(g.querySelectorAll('polygon.vector-head').length).toBe(1);
    const line = g.querySelector('line.vector-arrow')!;
